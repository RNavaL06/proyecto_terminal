    ...
    <div className="relative w-full max-w-lg mx-auto mb-4 z-[1000]">
      ...
      <div className="relative flex items-center">
        ...
        <input
          ...
          className="w-full pl-12 pr-4 py-3 bg-white border-2 border-slate-200 rounded-full shadow-sm focus:outline-none focus:border-blue-400 focus:ring-4 focus:ring-blue-50 transition-all text-slate-700 font-medium"
        />
      </div>
      ...
      {resultados.length > 0 && (
        <ul className="absolute top-full left-0 w-full mt-2 bg-white border border-slate-100 rounded-3xl shadow-lg overflow-hidden divide-y divide-slate-50 z-[10000]">
          {resultados.map((dir) => (
            <li
              key={dir.id}
              onClick={() => handleSelect(dir)}
              className="px-5 py-3 hover:bg-blue-50 cursor-pointer flex items-start gap-3 transition-colors"
            >
