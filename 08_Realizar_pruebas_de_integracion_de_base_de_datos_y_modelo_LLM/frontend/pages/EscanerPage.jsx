...
const EscanerPage = () => {
  ...
  const handleGuardarDatos = async (datosValidados) => {
    try {
      const respuesta = await guardarRecetaMedica(datosValidados);
      toast.success(respuesta.mensaje || '¡Receta guardada exitosamente en la base de datos!');
      setIsModalOpen(false);
      setDatosClinicos(null);
    } catch (err) {
      toast.error(err.message || 'Error al guardar la receta');
      throw err; // Re-lanzar para que el modal pueda manejar el estado de carga si lo requiere
    }
  };
