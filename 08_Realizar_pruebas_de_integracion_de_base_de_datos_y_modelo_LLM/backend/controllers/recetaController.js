...
const guardarReceta = async (req, res) => {
    ...
    try {
        ...
        // 4. Iterar el arreglo medicamentos y ejecutar un INSERT por cada uno
        if (Array.isArray(medicamentos) && medicamentos.length > 0) {
            for (const med of medicamentos) {
                const fecha_caducidad = parsearFecha(med.fecha_caducidad);

                await connection.query(
                    `INSERT INTO medicamentos 
                     (id_usuario, id_receta, nombre_comercial, sustancia_activa, dosis, formato, cantidad_disponible, fecha_caducidad, instrucciones_uso) 
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                    [
                        id_usuario,
                        id_receta,
                        med.nombre_comercial || "Sin nombre comercial",
